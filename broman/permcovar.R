...
if(bw) {
  pdf(file="../Figs/permcovar_bw.pdf", width=10, height=5, pointsize=12, onefile=TRUE)
  par(fg="black",col="black",col.axis="black",col.lab="black",
      bg=bgcolor, bty="n")
} else {
  pdf(file="../Figs/permcovar.pdf", width=10, height=5, pointsize=12, onefile=TRUE)
...
text((3*x2+x3)/4,y1/2,"phenotypes",srt=90)
text((x2+3*x3)/4,y1/2,"covariates",srt=90)
