...
- control residual variation to increase power
- look for QTL * covariate interactions

## additive covariate

$$\text{H}_0: y  =  \mu + \beta_x x + e$$  
$$\text{H}_a: y  =  \mu + \beta_x x + \beta_q q + e$$

- If covariate has strong effect on the phenotype, accounting for
...
- consider phenotype divided by covariate: $y/x$
- QTL model is $(y/x) = \mu + \beta_q q + e$
- equivalently

$$y = \left\{ \begin{array}{c@{\hspace{1cm}}l} \mu \ x + e & \text{if
      $q = 0$} \\
    (\mu + \beta_q) x + e & \text{if $q =
      1$} \end{array} \right. $$

## adjust then scan?

<img src="../Figs/y_over_x.jpg" alt="y over x" style="width: 500px"/>
